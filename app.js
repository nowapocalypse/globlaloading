...
function initLinks() {
    ...
    function setupLinkClick(btn, url) {
        if (!btn || !url) return;
        btn.href = url;
        ...
        btn.addEventListener("click", (e) => {
            e.preventDefault();
            e.stopPropagation();
            if (window.gmod && typeof window.gmod.openURL === "function") {
                window.gmod.openURL(url);
            } else {
    ...
    if (discordBtn && CONFIG.links.discord) {
        setupLinkClick(discordBtn, CONFIG.links.discord);
    }
    if (steamBtn && CONFIG.links.steamGroup) {
        setupLinkClick(steamBtn, CONFIG.links.steamGroup);
    }
}
...
function initAudioPlayer() {
    ...
    // Buton ve kontrol kapsülünün tamamına tek ve güvenilir tıklama olayı
    if (controlWrapper) {
